...
def video_to_frames(input_loc, output_loc):    
    ...
    while cap.isOpened():
        ...
        if (count > (video_length-1)):
            ...
            break
        # if want to stop at a frame number
        # if index == 100:
        #     break
...
def getCentroid( img, center_x, center_y, depth = 1, max_depth = 3 ):
    ...
    quarter_x   = round(len(img[0])/6,0)    #going this far left and right will give half image in total
    quarter_y   = round(len(img)/6,0)
...
def process_video( frame_dir, files, thetas= [0, 90, 180, 270]):
    ...
    if( os.path.basename(frame_dir) in centers.keys() ):
        ...
        ###################################################################################################################################
        #####################################################CHANGE FOR EVERY VIDEO!!!#####################################################
        ###################################################################################################################################
        #centroid        = getCentroid(img, 247, 277 )
        centroid        = getCentroid(img, 404, 260)
    ...
    for N, file in enumerate( files ):
       ...
       #     recalc_freq = 1
        ...
        #if ((N < 250 and N % 10 == 0) or N % recalc_freq == 0):
        ...
        if( (N < 250 and N % 10 == 0) or N % recalc_freq == 0):                                  #how often should we re-estaimte? constantly out the gate then reasonably regularly.
            ...
            if ((img[int(this_center_y), int(this_center_x)][0] < gradient_threshold and N % 1 == 0) or N % recalc_freq == 0):
                print(' gradient_threshold = ' + str(gradient_threshold))
            ...
            #lag is a parameter that should be tested. currently nAvg.
                ...
                if (gradient_threshold < 5):
                    this_center_x = int(np.mean(centers_x[max(-10, len(centers_x) - nAvg):]))
                    this_center_y = int(np.mean(centers_y[max(-10, len(centers_y) - nAvg):]))
